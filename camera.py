...
X_elip_cm = np.zeros(len(X_elip_px))
Y_elip_cm = np.zeros(len(Y_elip_px))

for i in range(0, len(X_elip_cm)):
    X_elip_cm[i] = tranf_image_coord_px_mm(X_elip_px[i])/10 # cm
    Y_elip_cm[i] = tranf_image_coord_px_mm(Y_elip_px[i])/10 # cm
